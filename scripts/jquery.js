...
var G = {};
(function( win, doc, notDefine ) {
    var loading       = {},                                                     // 正在载入中的模块，或者是依赖正载入中的模块
        waiting       = {},                                                     // 等待中的回调函数
        loaded        = {},                                                     // 已下载但是没有执行的脚本;
        module        = {},                                                     // 已req之后执行的脚本;
        config        = {
            url:'/js/lib/',                                                     // 不以 . 或 / 开头的模块名的路径前缀
            relativeUrl:'/js/'                                                  // 以.开头的模块名的路径路径前缀
        },                                                                      // 载入模块时候的url配置
     ...
     */
    G.ua = (function() {
        var isIE  = !+'\v1',                                                    // NOTE: 注意压缩之后 (\v1)会变成(\u000b1)，一定要替换回来 @_@!!
    ...
    /**
     * used by G.loadScript
     * @param {dom} node script节点
    ...
    function scriptOnload(node, callback) {
        ...
        node.addEventListener( 'error', function() {
          G.log('404 error:', node.src);
          callback();
        }, false);
    }
    if ( G.ua.isIE ) {
        scriptOnload = function(node, callback) {
            // NOTE: 在IE6/7/8中如果src指定的路径是404，你们不会触发error事件
     ...
     *               }
     * @returns {dom} script node
     */
    G.loadScript = function loadScript( url, callback ) {
     ...
     * @return {boolean} 是或否
     */
    G.isArray = nativeIsArray || function(obj) {
     ...
     * @param {function/object} wrap 模块wrapper或者是对象
     */
    G.def = function( name, deps, wrap ) {
        if ( (name in module) || (name in loaded) ) {
            return;
        }
        var tmp = wrap ? deps : notDefine;
        wrap = wrap || deps;
        deps = tmp;
        if ( !G.isFun( wrap ) ) {
            module[ name ] = wrap;
    ...
    /**
     * 加载已下载完成的模块，如果它有依赖则调用loadMod下载并加载
     ...
     * @returns {boolean/array} all modules
     */
    ...
    function nameToUrl( name ) {
        ...
        if ( isRelative ) {
            name = config.relativeUrl + name.slice(1);
        } else if ( !isAbsolute ) {
    ...
    function loadMod( name, callback ) {
        ...
        // 正在等待加载，或是等待依赖加载
        if ( name in loading ) { // 如果有循环引用会在这里停止，但是会存在waiting中，回调函数也不会执行
        ...
        // 还没开始下载
        G.loadScript( nameToUrl( name ), function( node ) {
            if ( name in module ) {
                clearMod( name, callback );
    ...
    /**
     * 加载已下载完成的模块，与exeMod不同。它的依赖也必须是已加载完成的
     * 如果没有下载则返回undefined
     * 这种用于处理模块已经下载的情况
     * 这个函数与以前的var xxx = G.req('xxx')写法类似，原函数总返回数组
     * 此修改之后，如果req的单个模块则不再返回数组，直接返回
     * @param {string} name 模块名字
     * @return {object} 对象模块，如果没有下载过则为undefined
     */
    ...
    /**
     * 将string根据空格分离成字符数组
     * @param {string/array} names 例如：'xxx xxxy'/'xxx'/['xx','xx2']
     * @return {array} 字符数组
     */
    function splitModuleNames( names ) {
        if ( G.isArray( names ) ) {
            return names;
        } else {
            return names.split(' ');
    ...
    /**
     * require module
     * @param {array/string} reqs 依赖的模块名
     * @param {function} callback 回调函数
     * @returns {array} 依赖的模块数组
     */
    G.req = function( reqs, callback ) {
        reqs = splitModuleNames( reqs );
        // 没有回调的情况，则只会加载已经下载的模块，如果没有下载则返回undefined
        if ( !callback ) {
            if ( reqs.length === 1 ) {
    ...
    G.format = function( tmpl, _key, _val ) {
        ...
        if ( typeof _key !== 'object' ) {
            ...
            return tmpl.replace(/\{([^{}]+)\}/g, function( match, key ) {
                val = obj[key];
                return ( val !== notDefine ) ? ( '' + val ) : '';
            });
        }
    };

    /**
     * 打印日志函数，自动判断是否有console，如果有则打印
     * 输入参数为当前浏览器支持的参数
     */
